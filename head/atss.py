...
def permute_and_flatten(layer, n, c, h, w):
    """
    Tile the spatial scale of the tensor to obtain the output tensor of shape=[batch_size, A*H*W, C]
    :param layer: The features of outputs. 某stage的输出特征图.
    :param n: batch_size
    :param c: channels of target.
    :param h: Height of features.
    :param w: Width of features.
    :return: The flatten of features.
    """
...
def concat_box_prediction_layers(box_cls, box_regression):
    """
    From https://github.com/sfzhang15/ATSS/blob/master/atss_core/modeling/rpn/utils.py
    N: batch_size.
    A: Number of instances predicted per patch.
    H: Height of pig.
    W: Width of pig.
    :param box_cls:
...
class ATSSHead(torch.nn.Module):
    """
    The master branch of each stage is divided into classification branch and
     regression master branch, and the regression master branch is divided into
      regression branch and central branch.
    """
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        if cfg.MODEL.ATSS.NUM_CONVS > 0:
            ...
            for i in range(cfg.MODEL.ATSS.NUM_CONVS):
                # 分类分支
                cls_tower.append(
                    nn.Conv2d(in_channels if i == 0 else channels, channels,
    ...
    def forward(self, x):
        ...
        for layer, feature in enumerate(x):
            ...
            logits.append(self.cls_logits(cls_tower))  # 将分类分支的结果

            bbox_pred = self.scales[layer](self.bbox_pred(box_tower))
...
class ATSS(nn.Module):
    ...
    @configurable
    def __init__(self, *, backbone: Backbone, head: nn.Module, head_in_features, anchor_generator, box2box_transform,
                 anchor_matcher, num_classes, focal_loss_alpha=0.25, focal_loss_gamma=2.0, box_reg_loss_weight=2.0,
                 pre_nms_thresh=0.05, pre_nms_top_n=1000, nms_thresh=0.6, max_detections_per_image=100, pixel_mean,
                 pixel_std, input_format="BGR", anchor_aspect_ratio, anchor_topk):
        """
        :param backbone:
        :param head:
        :param head_in_features:
        :param anchor_generator:
        :param box2box_transform:
        :param anchor_matcher:
        :param num_classes:
        :param focal_loss_alpha:
        :param focal_loss_gamma:
        :param box_reg_loss_weight:
        :param pre_nms_thresh:
        :param pre_nms_top_n:
        :param nms_thresh:
